...
- **v3.0 hybrid architecture — single-context evaluation retired.** A 2026-07-13 controlled comparison on a 73-skill library falsified the v2.0 premise that a large context window makes single-context auditing more accurate: the single-context pass returned all-Keep (84/84, including Keep verdicts on two paths that did not exist on disk), while fresh-context batches of 10–12 skills with **unconditional** reference verification surfaced 12 non-Keep verdicts (6 with deterministic evidence: 404 links, deleted files, retired CLI flags), and a dedicated overlap probe reproduced the set-level view without one giant context (0 genuine duplications across 17 candidate clusters, all documented layering). Root causes recorded in the design note: per-item attention dilutes as a context fills, and a *conditional* verification trigger ("confirm if it looks stale") degrades to "never verify" because the trigger itself is a diluting judgment. The audit is now split by the property being checked: Phase 0 deterministic structural pre-pass (skill-health scan + ledger hygiene, code-owned, mandatory), Phase 2 per-item scrutiny in parallel fresh-context batches (no prior verdicts passed — anchoring; no usage data — parent-owned), Phase 3 dedicated overlap probe (description sweep → side-by-side body reads; only confirmed genuine duplication produces Merge), Phase 4 parent synthesis, Phase 5 confirm-each consolidation. Requirements now include subagent (Task/Agent) support.
